...
class CaseParticipantForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "trustor": forms.Select(attrs={"class": "form-select js-trustor"}),
            "participant_type": forms.Select(attrs={"class": "form-select", "id": "id_participant_type"}),
            "represents": forms.Select(attrs={"class": "form-select js-represents", "id": "id_represents"}),
            "role": forms.Select(attrs={"class": "form-select"}),
            "main_participant": forms.CheckboxInput(attrs={"class": "form-check-input", "id": "id_main_participant"}),
    ...
    def clean(self):
        cleaned = super().clean()
        t = cleaned.get("participant_type")
        represents = cleaned.get("represents")
        person = cleaned.get("trustor")

        if t == "representative":
            if not represents:
                self.add_error("represents", "Укажите, кого представляет.")
            if person and represents and person.pk == represents.pk:
                self.add_error("represents", "Представитель не может представлять самого себя.")
            # представитель не может быть основным доверителем
            cleaned["main_participant"] = False
        else:
            cleaned["represents"] = None
